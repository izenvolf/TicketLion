...
<script type="text/javascript">
	...
	function check_and_submit(){
		if(document.TopOption.BYear.value==""){
			alert("년도를 선택해주세요.");
			
			document.TopOption.BYear.focus();
		}

		else if(document.TopOption.BMonth.value==""){
			alert("월을 선택해주세요");
			document.TopOption.BMonth.focus();
			
		}else{
			document.TopOption.submit();
					...
					<c:choose>
						<c:when test="${fn:length(list) > 0 }">
							<c:forEach items="${list }" var="val">
								<tr>
									<td><a href="#">${val.r_num }</a></td>
									<td>${val.reser_day }</td>
									<td>${fn:substring(val.view_date,0,16) } / ${val.reseat_num}</td>
								
									<td>${val.screen_name} / ${val.seat}</td>
										<!-- ${fn:substring(val.seat,0,1)}층 ${fn:substring(val.seat,8,9)}블럭 ${fn:substring(val.seat,2,4)}열  ${fn:substring(val.seat,6,8)} -->
